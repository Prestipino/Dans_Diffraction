...
class Plotting:
    ...
    def generate_nperiod_plane(self, HKL, inten, x_axis=[1,0,0], y_axis=[0,1,0],centre=[0,0,0], cut_width=0.05):
        """
        ...
        """
        # Lattice points and vectors within the plot
        Q_vec_a = self.xtl.Cell.calculateQ(x_axis)
        Q_vec_b = self.xtl.Cell.calculateQ(y_axis)
        ...
        q_max = np.max(np.sqrt(np.sum(Q * Q, axis=1))) * 1.1
        qmax2 = q_max * 2

        #CELL = np.array([qmax2 * x_cart, -qmax2 * y_cart, cut_width * z_cart ])

        #box_coord = fg.index_coordinates(Q - c_cart, CELL)
        #incell = np.all(np.abs(box_coord) <= 0.5, axis=1)
        #plane_coord = qmax2 * box_coord[incell, :]
        #qx, qy = plane_coord[:, 0], plane_coord[:, 1]
        #hkl_i, hkl =  inten[incell], HKL[incell, :]
        qx, qy, qz = Q.T
        hkl_i, hkl = inten, HKL
        ...
        plt.tricontourf(qx, qy, hkl_i, 100) #, cmap=cmap)
        plt.axis('image')
        plt.axis('equal')
        #plt.colorbar()
        ...
        #mesh_vec_a = fg.index_coordinates(Q_vec_a, CELL) * qmax2  # coordinates wrt plot axes
        #mesh_vec_b = fg.index_coordinates(Q_vec_b, CELL) * qmax2
        mesh_vec_a = Q_vec_a  # coordinates wrt plot axes
        mesh_vec_b = Q_vec_b
        ...
        fp.plot_vector_arrows(mesh_vec_a, mesh_vec_b, Q_vec_a_lab, Q_vec_b_lab)
